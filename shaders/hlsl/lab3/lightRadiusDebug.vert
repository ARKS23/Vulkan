...
VSOutput main(VSInput input, uint instanceID : SV_InstanceID) {
    ...
    float lightingRadius = max(light.intensity.w, 0.001f);
    float3 worldPosition = light.position.xyz + input.position * lightingRadius;

    output.position = mul(cameraInfo.projection, mul(cameraInfo.view, float4(worldPosition, 1.0f)));
    output.color = lerp(light.color.rgb, float3(1.0f, 1.0f, 1.0f), 0.35f);
    output.alpha = 0.55f;
